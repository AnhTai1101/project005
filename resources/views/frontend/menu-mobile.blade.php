<div class="menu-mobile">
    <ul class="topbar-mobile">
        <li>
            <div class="left-top-bar">
                Free shipping for standard order over $100
            </div>
        </li>

        <li>
            <div class="right-top-bar flex-w h-full">
                <a href="frontend/#" class="flex-c-m p-lr-10 trans-04">
                    Help & FAQs
                </a>

                <a href="frontend/#" class="flex-c-m p-lr-10 trans-04">
                    My Account
                </a>

                <a href="frontend/#" class="flex-c-m p-lr-10 trans-04">
                    EN
                </a>

                <a href="frontend/#" class="flex-c-m p-lr-10 trans-04">
                    USD
                </a>
            </div>
        </li>
    </ul>

    <ul class="main-menu-m">
        <li>
            <a href="frontend/index.html">Home</a>
            <ul class="sub-menu-m">
                <li><a href="frontend/index.html">Homepage 1</a></li>
                <li><a href="frontend/home-02.html">Homepage 2</a></li>
                <li><a href="frontend/home-03.html">Homepage 3</a></li>
            </ul>
            <span class="arrow-main-menu-m">
                <i class="fa fa-angle-right" aria-hidden="true"></i>
            </span>
        </li>

        <li>
            <a href="frontend/product.html">Shop</a>
        </li>

        <li>
            <a href="frontend/shoping-cart.html" class="label1 rs1" data-label1="hot">Features</a>
        </li>

        <li>
            <a href="frontend/blog.html">Blog</a>
        </li>

        <li>
            <a href="frontend/about.html">About</a>
        </li>

        <li>
            <a href="frontend/contact.html">Contact</a>
        </li>
    </ul>
</div>